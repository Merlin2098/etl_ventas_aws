...
└── modules/
    ├── s3_data_lake/          # buckets bronze, silver, gold, quarantine
    ├── lambda_ingestion/       # 8 funciones Lambda (4 ingesta + 4 transform) + sus IAM roles + event notifications
...
- Una única notificación S3 (`aws_s3_bucket_notification`, AWS solo permite un recurso de este tipo por bucket) sobre el bucket de datos, con dos conjuntos de bloques `lambda_function`:
  - Uno por división para la Lambda de **ingesta**, filtrado por `filter_prefix = "bronze/<division>/"` (ej. `filter_prefix = "bronze/electronica/"`). Al ser la división el primer segmento del path (no la fecha, que es variable), el prefijo es literal y estático — condición necesaria para que `filter_prefix` funcione (S3 no admite comodines en filtros de notificación).
  - Uno por división para la Lambda de **transformación**, filtrado por `filter_prefix = "silver/store=<division>/"` (mismo razonamiento, aplicado al prefijo Silver).
...
- Siguiendo **Policy 010** (IAM cross-module placement): el permiso `lambda:InvokeFunction` que S3 necesita para invocar cada Lambda (`aws_lambda_permission`, con `source_arn` apuntando al bucket) se declara en `modules/lambda_ingestion` (donde viven las funciones), no en `modules/s3_data_lake`. El módulo S3 recibe los ARNs de las 8 Lambdas (como dos maps: ingesta y transformación) como variables de entrada para configurar `aws_s3_bucket_notification`.
...
- `aws_s3_bucket_notification` (una sola, con bloques de ingesta y transformación)
...
- `s3_data_lake` depende de `lambda_ingestion` (necesita los ARNs de las 8 funciones, ingesta y transformación, para configurar la notificación única y otorgar `lambda:InvokeFunction`, ver Policy 010).
...
5. **Segundo `terraform apply`**: crea `lambda_ingestion` (8 funciones), `s3_data_lake` (con la notificación única ya apuntando a las 8 Lambdas existentes), `glue_catalog` y `athena`.
